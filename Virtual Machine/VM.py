...
Basically a lot of niceties left undefined on the bare-bones, two-register Hack architecture, which sets up structures that correspond very well to C-style/"structured programming" language features.

The converter is non-optimizing, but the introduction of some extra instructions could make the output much more time & space efficient..
...
class Output(object):
    ...
    out(string)
        This is raison d'être of the entire class. We use it for all of our...output. '''
...
I kind of hate this code, but it's fundamentally some /really/ simple text processing, and it does what it should. Also, the documentation \
makes pretty clear what the translation's actually doing.
...
Main function'''
    ...
    for file in filelist:
        ...
        linenum = 1
        for line in read:
            line = tokenize(line) # `line' is now a list of operators and arguments.
...
def processArithmeticLogic(line, linenum, gID):
    ...
    ### UPDATE: tried this, and it offered no performance update and somewhat obfuscated the code
    if len(line) != 1:
...
def processProgramFlow(line, module):
    ...
    if len(line) != 2:
        print("%s commands accept one argument, or your label name contains a space." % line[0], file=sys.stderr)
    else:
        # My implementation of the command `label /label/` -> functionName$label
        #  assumes the compiler has generated unique (and correct) names for labels.
